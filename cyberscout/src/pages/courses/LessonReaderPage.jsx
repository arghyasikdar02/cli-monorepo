...
export default function LessonReaderPage() {
  ...
  if (!course.enrolled) return (
    <AppShell focusMode>
      <div className="max-w-[720px] mx-auto px-8 py-20 text-center">
        <SiteIcon name="lock" size={54} className="mx-auto mb-3 text-slate-300" />
          ...
          <div className="sticky top-20">
            ...
            {lessons.map(item => (
              <Link key={item.id} to={`/lessons/${courseId}/${item.id}`}
                ...
                {item.completed
                  ? <SiteIcon name="check_circle" size={14} className="text-green-500" />

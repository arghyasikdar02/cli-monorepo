...
- Backend: Render, root directory `cyberscout-server`, build command `npm ci`, start command `npm run db:migrate && npm start`.
- Database: Supabase PostgreSQL, configured only through the backend `DATABASE_URL` secret.

Render runs pending tracked migrations before it starts Express. It does not seed or reset data and does not use a persistent disk. Configure variables from `/cyberscout-server/.env.example` and see [`SUPABASE_POSTGRESQL.md`](SUPABASE_POSTGRESQL.md).
